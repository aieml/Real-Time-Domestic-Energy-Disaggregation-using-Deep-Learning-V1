...
train_x=np.zeros((k,T,m),dtype=np.float32)
train_x_bar=np.zeros((T,m),dtype=np.float32)
test_x=np.zeros((k,T1,m),dtype=np.float32)
test_x_bar=np.zeros((T1,m),dtype=np.float32)
...
for j in range(T):
    ...
    train_x_bar[j][0]=train_x[0][j][0]+train_x[1][j][0]+train_x[2][j][0]+train_x[3][j][0]+5*abs(np.random.randn(1))

for j in range(T1):
    
    test_x_bar[j][0]=test_x[0][j][0]+test_x[1][j][0]+test_x[2][j][0]+test_x[3][j][0]+5*abs(np.random.randn(1))
...
pickle.dump(train_x_bar,open('train_x_bar.pickle','wb'))
...
for j in range (k):
    ...
    while (cp.norm(x - cp.matmul(B_Update,A_Update),"fro").value >= 0.01):
...
count=0
while (cp.norm(x_tot - cp.matmul(B_Bar_H,A_Str_V),"fro").value >= 0.01):
